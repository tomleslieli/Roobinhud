...
const SellStockForm = ({
  ...
  const handleSubmit = (buyIn, amount, currentPrice, estimatedCost) => {
    ...
    if ((buyIn === 'Shares' && amount <= portfolio.quantity) || (buyIn === 'Dollars' && (parseFloat((amount / currentPrice).toFixed(5)) <= portfolio.quantity))) {
      userObj.buying_power = user.buying_power + parseFloat(estimatedCost)
      if (buyIn === "Dollars") {
        portfolio.quantity = parseFloat(
          (portfolio.quantity - quantity).toFixed(5)
        );
      } else {
        portfolio.quantity = parseFloat(
          (portfolio.quantity - parseFloat(amount)).toFixed(5)
        );
      }
  ...
  return (
    <div className="transaction-form-container">
      ...
      >
        ...
        {buyIn === "Shares" ? (
          <div className="transaction-form-row">
            <h6 className="market-price-sell">Market Price</h6>
      ...
      { errorMessage ?
        <div className="sell-error">
          <h6 id='sell-error-text'>Not enough shares.</h6>
        </div>
        : <></>
      }
    </div>
...
export default connect(mSTP, mDTP)(SellStockForm);
